...
A aba **Trocar roupa** encadeia duas etapas locais: o FLUX veste a peça escolhida na foto da influencer (enviada ou vinda da galeria da Influencer IA) e a foto já vestida segue direto para o Wan Animate com o vídeo de dança. O seletor **Produto · apresentar** troca o vestir por fazer a influencer **segurar e apresentar qualquer produto** na câmera (modo `product` no campo `mode` da API), que é a base dos vídeos de vendas. Os pesos do FLUX podem ser trocados por `CUTCLIPS_FLUX_MODEL`, `CUTCLIPS_FLUX_TEXT_ENCODER` e `CUTCLIPS_FLUX_VAE`; o `GET /api/motion-control/config` devolve `flux_ready`/`flux_missing` para a interface dizer o que falta.

A aba **Falar (lip-sync)** sincroniza os lábios da foto com um áudio enviado (WAV/MP3/M4A/AAC/OGG/FLAC de 1 a 60 segundos), com um texto escrito ou com uma **resposta a comentários**: cole os comentários e a IA (ou, sem chave, um template agradecido) escreve a resposta da influencer, que é falada com a voz escolhida (`reply_to` no `POST /api/motion-control/lipsync`, até 2000 caracteres). Não há fluxo embutido: exporte um fluxo de lip-sync (LatentSync, Wav2Lip, SadTalker…) em formato API com um `LoadImage`, um `LoadAudio` e exatamente um `SaveVideo`, e aponte `CUTCLIPS_LIPSYNC_WORKFLOW` para o JSON. O texto é sintetizado antes de qualquer envio ao ComfyUI, então erro de voz aparece em segundos.
...
**Progresso de verdade**: o rótulo de cada etapa mostra o percentual real (`Gerando vídeo · 42%`), lido do WebSocket do ComfyUI — uma conexão global única por instalação serve todos os jobs (cada espera lê só o `prompt_id` que submeteu) e a porcentagem vai para o `status.json` a cada 5% para não reescrever o arquivo a cada evento. Sem ComfyUI, a etapa segue apenas com o texto.

**Passe de refino (mãos e rótulo)**: nas abas **Trocar roupa** e **Vender**, a caixa *Passe extra do FLUX para corrigir mãos e rótulo* liga uma segunda passada dentro da etapa 1: a foto recém-composta vira a referência de identidade (imagem 1) e o FLUX refaz a cena com instrução de corrigir mãos e texto de rótulo, mantendo composição, pessoa e fundo. Custa ~40 s a mais, fica desligada por padrão e sobrevive ao retry (`refine` no `status.json`).

**Lip-sync sobre o vídeo de dança (terceira saída)**: com `CUTCLIPS_LIPSYNC_VIDEO_WORKFLOW` apontando para um fluxo de lip-sync sobre vídeo (um carregador `LoadVideo` ou `VHS_LoadVideo` + um `LoadAudio` + um `SaveVideo`), o one-shot entrega um terceiro vídeo — a influencer **dançando e falando ao mesmo tempo** (`dance_talk.mp4`, *Vídeo 3* na tela, rotas `GET /api/motion-control/{id}/dance-talk[/download]`). O fluxo é validado na criação do job (503 antes de qualquer render), aparece no painel de configuração quando declarado mas incompleto, e a etapa tem bandeira própria de retomada (`done_dance_talk`).

**Voz sobre a dança (`voiced.mp4`)**: o one-shot entrega de graça, em segundos e sem rodar IA, a dança com a voz de venda por cima (`GET /api/motion-control/{id}/voiced[/download]`, *Vídeo 1 com a voz* na tela). O ffmpeg abafa o áudio da dança enquanto ela fala (`sidechaincompress`) e, se a fala for mais longa que a dança, o vídeo repete até o fim da voz — é a prévia de vendas imediata enquanto o lip-sync de vídeo renderiza (ou quando ele não está configurado). Etapa com bandeira própria (`done_voiced`).

**Card de preço queimado**: na aba Vender, o campo *Preço* gera um `price.ass` local — pílula `#ffdd45` fixa no topo, texto escuro, tamanho proporcional ao vídeo — e queima nos **todos** os vídeos do job com `ffmpeg -vf subtitles`, sempre por último (o `result` é entrada do voiced e do `dance_talk`, e queimar cedo duplicaria a pílula). Falha cedo (503) se o ffmpeg não tiver o filtro `subtitles`, e cada vídeo pronto ganha um marcador `.priced`: o retry só aplica o preço no que faltou.
